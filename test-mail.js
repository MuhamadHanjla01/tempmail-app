const { createAccount, login, getMessages } = require('./src/lib/mail');
...
async function test() {
    ...
    // Simulate someone sending an email or just wait, actually we can't send an email easily here unless we use another API.
    // Let's just check if login returns a token that can fetch messages.
